...
from .scoped_symbols import SCOPED_SYMBOL_REVISION, ScopedSymbolID
...
_FIELD_NAMES = (
    ...
    "runtime_guard", "name", "value", "axes", "axis", "keepdims", "callee", "trip_count",
    "body_kind",
)
...
_T_GLOBAL = 0x0A
_T_DOMAIN_TEXT = 0x0B
_T_SCOPED_SYMBOL = 0x0C


@dataclass(frozen=True)
class ScopedSymbolRef:
    scope_sid: bytes
    symbol_id: ScopedSymbolID

    def __post_init__(self) -> None:
        scope = bytes(self.scope_sid)
        if len(scope) != 16:
            raise TypeError("ScopedSymbolRef scope must be a 128-bit StructuralID")
        object.__setattr__(self, "scope_sid", scope)
        if not isinstance(self.symbol_id, ScopedSymbolID):
            object.__setattr__(self, "symbol_id", ScopedSymbolID(bytes(self.symbol_id)))
...
def _prepare(
    ...
    *,
    ...
    if value is None or isinstance(value, (bool, int, float, bytes, DomainText, ScopedSymbolRef)):
        return value
    if isinstance(value, str):
        ...
        if not as_key:
            ref = _global_ref(
                ...
                container=container if isinstance(container, Mapping) else None,
            )
            if ref is not None:
                return _GlobalRef(ref.domain, ref.code)
        raise TypeError(f"NSM6 refuses untyped text: {value!r} at field {field_name!r}")
    if isinstance(value, Mapping):
        out: dict[Any, Any] = {}
        for key, item in value.items():
            prepared_key = _prepare(key, as_key=True, container=value)
            key_name = key if isinstance(key, str) else None
            out[prepared_key] = _prepare(item, field_name=key_name, container=value)
        return out
    if isinstance(value, (list, tuple)):
        return [_prepare(item, field_name=field_name, container=container) for item in value]
    raise TypeError(f"unsupported NSM6 value type: {type(value).__name__}")
...
def _encode(value: Any) -> bytes:
    ...
    if isinstance(value, float):
        if not math.isfinite(value): raise ValueError("NSM6 rejects NaN and Infinity")
    ...
    if isinstance(value, ScopedSymbolRef):
        return bytes((_T_SCOPED_SYMBOL,)) + value.scope_sid + value.symbol_id.raw
    ...
    if isinstance(value, Mapping):
        ...
        return bytes(out)
    raise TypeError(f"unsupported NSM6 prepared type: {type(value).__name__}")


def _decode(data: memoryview, offset: int) -> tuple[Any, int]:
    if offset >= len(data): raise DecodeError("truncated NSM6 value")
    ...
    if tag == _T_FLOAT64:
        ...
        if end > len(data): raise DecodeError("truncated NSM6 float")
        return struct.unpack(">d", data[offset:end])[0], end
    if tag == _T_BYTES:
        length, offset = _read_uvarint(data, offset); end = offset + length
        if end > len(data): raise DecodeError("truncated NSM6 bytes")
        return bytes(data[offset:end]), end
    if tag == _T_FIELD:
        code, offset = _read_uvarint(data, offset)
        try: return _CODE_TO_FIELD[code], offset
        except KeyError as exc: raise DecodeError(f"unknown NSM6 field code: {code}") from exc
    if tag == _T_GLOBAL:
    ...
    if tag == _T_DOMAIN_TEXT:
        ...
        if end > len(data): raise DecodeError("truncated NSM6 domain text")
        try: text = bytes(data[offset:end]).decode("utf-8")
        except UnicodeDecodeError as exc: raise DecodeError("invalid UTF-8 in NSM6 domain text") from exc
        try: domain = domain_from_code(domain_code)
        except KeyError as exc: raise DecodeError(str(exc)) from exc
        return DomainText(domain, text), end
    if tag == _T_SCOPED_SYMBOL:
        end = offset + 32
        if end > len(data): raise DecodeError("truncated NSM6 scoped symbol reference")
        scope = bytes(data[offset:offset+16])
        symbol = ScopedSymbolID(bytes(data[offset+16:end]))
        return ScopedSymbolRef(scope, symbol), end
    if tag == _T_ARRAY:
    ...
    if tag == _T_MAP:
        ...
        for _ in range(length):
            ...
            if key in items: raise DecodeError("duplicate NSM6 map key")
            items[key] = item
        return items, offset
    raise DecodeError(f"unknown NSM6 tag: {tag}")


def _contains_textual_scoped_symbol(value: Any) -> bool:
    if isinstance(value, DomainText):
        return value.domain.value == "scoped_semantic_symbol"
    if isinstance(value, Mapping):
        return any(_contains_textual_scoped_symbol(k) or _contains_textual_scoped_symbol(v) for k, v in value.items())
    if isinstance(value, (list, tuple)):
        return any(_contains_textual_scoped_symbol(v) for v in value)
    return False


def encode_scoped_record(record: Any) -> bytes:
    if contains_human_projection(record):
        raise TypeError("NSM6 refuses Human Projection inside canonical semantic payload")
    if _contains_textual_scoped_symbol(record):
        raise TypeError("NSM6 refuses textual scoped semantic symbols")
    ...
    return (
        ...
        + _uvarint(SCOPED_SYMBOL_REVISION)
        + body
    )


def decode_scoped_record(blob: bytes | bytearray | memoryview) -> Any:
    data = memoryview(blob)
    if len(data) < 4 or bytes(data[:4]) != _MAGIC:
        raise DecodeError("invalid NSM6 magic")
    offset = 4
    registry_revision, offset = _read_uvarint(data, offset)
    if registry_revision > REGISTRY_REVISION:
        raise DecodeError("NSM6 registry revision is newer than supported")
    domain_revision, offset = _read_uvarint(data, offset)
    if domain_revision != DOMAIN_REVISION:
        raise DecodeError("unsupported NSM6 symbol-domain revision")
    purification_revision, offset = _read_uvarint(data, offset)
    if purification_revision != PURIFICATION_REVISION:
        raise DecodeError("unsupported NSM6 purification revision")
    scoped_revision, offset = _read_uvarint(data, offset)
    if scoped_revision != SCOPED_SYMBOL_REVISION:
        raise DecodeError("unsupported NSM6 scoped-symbol revision")
    value, offset = _decode(data, offset)
    if offset != len(data):
        raise DecodeError("trailing bytes after NSM6 record")
    if contains_human_projection(value):
        raise DecodeError("NSM6 payload contains forbidden Human Projection")
    if _contains_textual_scoped_symbol(value):
        raise DecodeError("NSM6 payload contains textual scoped semantic symbols")
    return value


__all__ = ["ScopedSymbolRef", "decode_scoped_record", "encode_scoped_record"]
